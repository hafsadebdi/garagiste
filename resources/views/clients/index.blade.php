    ...
    <section class="content">
            ...
            <div class='max-w-md mx-auto mt-10'>
                    <div class="relative flex items-center w-full h-12 rounded-lg focus-within:shadow-lg bg-white overflow-hidden">
                        ...
                        <form id="searchForm" onsubmit="return submitForm(event)" action="{{ route('clients.search') }}" method="post" >
                            @csrf
                            <input  class="peer h-full w-full outline-none text-sm text-gray-700 pr-2"
                            type="text"  id="search" name="search" placeholder="@lang('Rechercher quelque chose..')" />
                        </form>
                    </div>
            </div>
            <div id="divResult">
...
function submitForm(event){
        ...
        var url = "{{ route('clients.search')}}"   //  $("#searchForm").attr("action");
